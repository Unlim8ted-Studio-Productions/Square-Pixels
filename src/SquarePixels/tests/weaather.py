...
def draw_rain(screen, raindrops, wind):
    ...
    raindrops[:] = [
        drop for drop in raindrops if drop[1] < infoObject.current_h and drop[2] > 0
    ]
...
def update_weather(weather, raindrops, lightning_pos, clouds, leaves, wind):
    if weather == "rain":
        for i in range(10):
            ...
            raindrops.append(
                [
                    random.randint(0, infoObject.current_w),
                    random.randint(0, infoObject.current_h),
                    255,
                ]
            )
