...
def draw_text(text, x, y, font_size_factor=1, color=black):
...
while runningg:
    ...
    # Draw text
    draw_text("Photosensitive Seizure Warning", width * 0.1, height * 0.07)
    draw_text(
    ...
    draw_text("than adults to experience these seizures.", width * 0.07, height * 0.43)
    draw_text(
        "You may reduce the risk of photosensitive epileptic seizures by taking the following precautions:",
        width * 0.07,
        height * 0.48,
    )
    draw_text("- View in a well-lit room.", width * 0.09, height * 0.51)
    draw_text(
        "- Do not view if you are drowsy or fatigued.", width * 0.09, height * 0.54
    )
    draw_text(
