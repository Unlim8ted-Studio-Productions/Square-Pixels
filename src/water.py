...
def create_walls(leftwallpos,rightwallpos,size):
    ...
    wall_left.position = leftwallpos
    wall_left_shape = pymunk.Poly.create_box(wall_left, size)
    space.add(wall_left, wall_left_shape)

    wall_right = pymunk.Body(mass=10 ** 5, moment=0, body_type=pymunk.Body.STATIC)
    wall_right.position = rightwallpos
    wall_right_shape = pymunk.Poly.create_box(wall_right, size)
...
def main_game():
    ...
    if USE_PYMUNK:
        create_walls((200,300),(600,100),(200,400))
    wave = Wave()
