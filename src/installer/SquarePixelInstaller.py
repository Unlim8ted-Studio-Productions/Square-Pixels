...
def urlretrieve(url, filename=None, reporthook=None, data=None, tpcobject=None):
    """
    ...
    with contextlib.closing(urlopen(url, data)) as fp:
        ...
        if tpcobject != None:
            ...
            )  # Label for installation details
            details_label.pack()
            # details_ = tkinter.Label(tpcobject, text="", pady=10)
            # details_.pack()
        with tfp:
    ...
    def do_request_(self, request):
        ...
        if request.data is not None:  # POST
            ...
            if isinstance(data, str):
                msg = (
                    "POST data should be bytes, an iterable of bytes, "
...
def create_start_menu_shortcut(extract_path, shortcut_name):
    ...
    shortcut.IconLocation = (
        extract_path
        + "\\terraria_styled_game\PE EXE file compile recources\icons scalable.ico"  # Set the icon location
    )
    shortcut.save()
